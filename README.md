...
| 一次过生成 10 秒，不要拆分 | A0 SinglePass 编译（自动编译器 + 7维信息校验） |
...
| 出稿后看看怎么更有冲击力 | PIA 审计：全网搜索同类镜头 + 戏剧化/冲击力提升建议 |
| 用 Seedance 跑，明代古风坠崖慢动作 | Seedance 原生语法适配 + 真实感视角库 |
...
| A0 单次编译 | prompts.single_pass（7维信息无损，`scripts/compile-single-pass.js` 自动编译器 + 声锚转 `<>` + 拍N标签保留） |
| 惊艳四件套 | 真声轨 + 微动节拍 + 光相位 + 首尾帧（按模型能力启用） |
| Seedance 适配 | 原生语法（`()`音乐 / `<>`音效 / `{}`台词 / `【】`字幕）+ @槽位负面声明 + FLF 首帧链 |
| 真实感视角 | 手机直拍 / dashcam / CCTV / 无人机 / 采访直播 / found footage |
| **出稿前审核** | **七维逻辑审核**（合同一致性/空间拓扑/情绪因果/物理尺度/声画对齐/密度/一次看懂） |
| **出稿后审计** | **PIA 冲击力审计**（全网搜索同类镜头 + 戏剧化/冲击力/感染力提升建议，用户确认后才改） |
...
| **一次过生成** | **手动重写丢信息** | **A0 自动编译器（7维校验 + 信息无损）** |
| **出稿前审核** | **没有** | **七维逻辑审核：合同矛盾/机位瞬移/情绪断裂在提交前拦截** |
| **出稿后审计** | **没有** | **PIA 冲击力审计：全网搜索同类镜头 + 提升建议** |
