...
### U4b · 场景维度（场景四问 · 确认式 · 2026-08-16 新增）

> 方法论：`references/narrative-spine.md` §9 场景维度推断表

基于 U0 意图关键词匹配场景类型 → 自动注入环境纵深/生活细节/色彩/氛围模板值，标注 `[推断]`：

- **S1 空间布局**：前景/中景/远景各有什么？视觉中心在哪？（展示模板后问"需要调整吗？"）
- **S2 生活细节**：场景中有谁在活动？有什么道具陈设？（展示模板后问"需要加/减吗？"）
- **S3 色彩色调**：主色调偏冷还是暖？饱和度高低？（确认式："模板建议冷蓝清爽，可以吗？"）
- **S4 整体氛围**：想接近哪个导演/电影的质感？（确认式："模板建议岩井俊二式治愈，可以吗？"）

**静默代填规则**：意图含场景关键词 → 模板注入 `[推断]`；用户确认 → `[锁定]`；用户否决 → 给 2-3 个替代；用户补充 → 覆盖模板值；意图不含已知关键词 → S1-S4 全开放式。
...
展示锁定卡：人 / 衣帽 / 道具归属 / 环境纵深 / 生活细节 / 色彩色调 / 画质质感 / 整体氛围 / 材质 / overview 要点。
